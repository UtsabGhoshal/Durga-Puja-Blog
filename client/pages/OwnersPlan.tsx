...
import { Calendar, MapPin, Navigation, ExternalLink, Clock, Star, Route, User, Heart } from "lucide-react";
      ...
      >
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              { value: "1", label: "Day Documented", icon: <Calendar className="w-8 h-8" />, color: "from-festival-orange to-festival-saffron" },
              { value: "15+", label: "Pandals Listed", icon: <Star className="w-8 h-8" />, color: "from-blue-500 to-blue-600" },
              { value: "2", label: "Key Areas", icon: <MapPin className="w-8 h-8" />, color: "from-emerald-500 to-green-600" },
              { value: "3", label: "Route Options", icon: <Route className="w-8 h-8" />, color: "from-purple-500 to-indigo-600" }
            ].map((stat, index) => (
              <motion.div
                key={index}
